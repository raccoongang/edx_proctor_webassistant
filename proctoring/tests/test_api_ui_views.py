...
class ViewsUITestCase(TestCase):
    ...
    def test_start_exam(self):
        ...
        with patch('proctoring.api_ui_views.start_exam_request') as edx_request, \
                patch('proctoring.api_ui_views.send_ws_msg') as send_ws:
            edx_request.return_value = MockResponse()
            send_ws.return_value = None
            request = factory.get(
                '/api/start_exam/%s' % self.exam.exam_code)
            force_authenticate(request, user=self.user)
            response = api_ui_views.start_exam(request,
    ...
    def test_stop_exam(self):
        ...
        with patch('proctoring.api_ui_views.stop_exam_request') as edx_request:
            ...
            force_authenticate(request, user=self.user)
            response = api_ui_views.stop_exam(request,
                                          attempt_code=self.exam.exam_code)
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
            response.render()
            data = {
    ...
    def test_stop_exams(self):
        ...
        force_authenticate(request, user=self.user)
        response = api_ui_views.stop_exams(request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_poll_status(self):
        ...
        with patch('proctoring.api_ui_views.poll_status_request') as edx_request, \
                patch('proctoring.api_ui_views.send_ws_msg') as send_ws:
            ...
            force_authenticate(request, user=self.user)
            response = api_ui_views.poll_status(request)
            response.render()
            self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_send_review(self):
        ...
        with patch('proctoring.api_ui_views.send_review_request') as edx_request:
            edx_request.return_value = MockResponse()
            data = {
                "desktopComments": """[
                        {
